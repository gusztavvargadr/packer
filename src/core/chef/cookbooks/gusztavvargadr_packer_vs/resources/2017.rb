...
action :install do
  directory_path = "#{Chef::Config[:file_cache_path]}/gusztavvargadr_packer_vs/2017_#{edition}"
  ...
  installer_file_source = node['gusztavvargadr_packer_vs']["2017_#{edition}"]['installer_file_url']
  remote_file installer_file_path do
    source installer_file_source
    action :create
  end

  installer_options = node['gusztavvargadr_packer_vs']["2017_#{edition}"]['installer_options'].join(' ')
