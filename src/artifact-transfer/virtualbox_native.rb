...
module ArtifactTransfer
  module VirtualBoxNative
    ...
    PREPARE_NATIVE_COMMAND = 'prepare-virtualbox-native'.freeze
    PREPARE_VAGRANT_COMMAND = 'prepare-virtualbox-vagrant'.freeze
    COMPLETE_VAGRANT_COMMAND = 'complete-virtualbox-vagrant'.freeze
    ...
    def promote_image(canonical, image)
    ...
    def cleanup_promoted_image(image, promoted, manifest = nil)
      return unless promoted

      FileUtils.rm_rf(image)
      FileUtils.rm_f(manifest) unless manifest.nil?
    end

    def prepare_virtualbox_native(artifact_root)
      ...
      begin
        ...
        cleanup_promoted_image(image, promoted, File.join(artifact_root, VIRTUALBOX_MANIFEST_FILENAME))
        raise
      end
    end
    ...
    def prepare_virtualbox_vagrant(artifact_root)
      ...
      begin
        ...
        cleanup_promoted_image(image, promoted)
        raise
      ensure
    ...
    def complete_virtualbox_vagrant(artifact_root)
      artifact_root = File.expand_path(artifact_root)
      image = File.join(artifact_root, 'image')
    ...
    def run(arguments)
      ...
      when [PREPARE_NATIVE_COMMAND, 1]
        prepare_virtualbox_native(command_arguments.first)
      when [PREPARE_VAGRANT_COMMAND, 1]
        prepare_virtualbox_vagrant(command_arguments.first)
      when [COMPLETE_VAGRANT_COMMAND, 1]
        complete_virtualbox_vagrant(command_arguments.first)
      else
...
exit ArtifactTransfer::VirtualBoxNative.run(ARGV)
